...
const statusConfig = {
  activo: { label: 'Activo', color: 'text-blue-400', bg: 'bg-blue-400/10 border-blue-400/20' },
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-white mb-1">Evaluaciones</h1>
            <p className="text-slate-400 text-sm">{EVALS.length} evaluaciones · promedio general {Math.round(EVALS.reduce((a, e) => a + e.avgScore, 0) / EVALS.length)}%</p>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Evaluaciones', value: EVALS.length, color: 'text-blue-400' },
      ...
      <div className="grid lg:grid-cols-3 gap-5">
              ...
              <motion.div key={ev.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-xs mt-0.5">{ev.questions} preguntas · Vence {ev.due}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-3">
                  <div className="text-center">
                    <div className="text-lg font-black text-white">{ev.avgScore}%</div>
                    <div className="text-slate-500 text-xs">Promedio</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-black text-emerald-400">{ev.passed}</div>
                    <div className="text-slate-500 text-xs">Aprobados</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-black text-blue-400">{ev.total}</div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-1.5 bg-white/8 rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${passRate}%` }} transition={{ duration: 1, delay: 0.5 }}
                      className={`h-full rounded-full ${passRate >= 90 ? 'bg-emerald-500' : passRate >= 70 ? 'bg-blue-500' : 'bg-orange-500'}`} />
                  </div>
                  <span className="text-xs text-slate-400">{passRate}% aprobados</span>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <div className="px-5 py-4 border-b border-white/8">
            <h3 className="font-bold text-white text-sm">Resultados Recientes</h3>
          </div>
              ...
              <div key={i} className="px-5 py-3.5 hover:bg-white/2 transition-colors">
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="text-slate-500 text-xs truncate">{r.eval}</div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/8">
              <h2 className="text-white font-bold">Nueva Evaluación</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="text-slate-400 text-xs font-semibold mb-1.5 block">Capacitación asociada</label>
            ...
            <div className="px-6 pb-6 flex gap-3">
              ...
              <button onClick={() => setShowModal(false)}
                className="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold transition-all">Crear Evaluación</button>
            </div>
          </motion.div>
        </div>
      )}
