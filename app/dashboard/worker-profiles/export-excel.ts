...
export async function exportToExcel(profiles: WP[], analytics: Analytics, company = 'Empresa') {
  ...
  const base = profiles.map(p => ({
    ...
    'TIPO DOCUMENTO':             p.doc_type ?? '',
    'USUARIO / EMAIL DE INGRESO': p.users?.email ?? '',
    'CORREO PERSONAL':            p.email_personal ?? '',
    'TELÉFONO':                   p.telefono ?? '',
    'TELÉFONO ALTERNO':           p.telefono_alterno ?? '',
    'FECHA NACIMIENTO':           p.fecha_nacimiento ?? '',
    'EDAD (años)':                age(p.fecha_nacimiento),
    'CIUDAD NACIMIENTO':          p.ciudad_nacimiento ?? '',
    'DEPTO NACIMIENTO':           p.depto_nacimiento ?? '',
    'NACIONALIDAD':               p.nacionalidad ?? '',
    'SEXO':                       p.sexo ?? '',
    'GRUPO SANGUÍNEO Y RH':       p.grupo_sanguineo ?? '',
    ...
    'PARENTESCO':                 p.parentesco_contacto ?? '',
    'TEL. EMERGENCIA':            p.tel_contacto ?? '',
    'SEGUNDO CONTACTO':           p.contacto_emergencia2 ?? '',
    'PARENTESCO 2':               p.parentesco_contacto2 ?? '',
    'TEL. SEGUNDO CONTACTO':      p.tel_contacto2 ?? '',
    'EPS':                        p.eps ?? '',
    'ARL':                        p.arl ?? '',
    'FONDO DE PENSIONES':         p.fondo_pension ?? '',
    'CAJA DE COMPENSACIÓN':       p.caja_compensacion ?? '',
    'TIENE DISCAPACIDAD':         bool(p.tiene_discapacidad),
    'CUÁL DISCAPACIDAD':          p.discapacidad_detalle ?? '',
    'POBLACIÓN VULNERABLE':       p.poblacion_vulnerable ?? '',
    ...
    'ESTUDIOS TÉCNICOS':          p.estudios_tecnicos ?? '',
    'ESTUDIOS TECNOLÓGICOS':      p.estudios_tecnologicos ?? '',
    'ESTUDIOS UNIVERSITARIOS':    p.estudios_universitarios ?? '',
    'ESPECIALIZACIÓN':            p.especializacion ?? '',
    'OTROS ESTUDIOS':             p.otros_estudios ?? '',
    'CURSOS Y CERTIFICADOS':      p.cursos_certificados ?? '',
    'CARGO':                      p.cargo_confirmado ?? '',
    'ÁREA':                       p.area_confirmada ?? p.users?.area ?? '',
    'CENTRO TRABAJO / SEDE':      p.centro_trabajo ?? '',
    'JEFE INMEDIATO':             p.jefe_inmediato ?? '',
    ...
    'DISTANCIA APROX.':           p.distancia_aprox ?? '',
    ...
    'TALLA CAMISA / CAMISETA':    p.talla_camisa ?? '',
    ...
    'OBSERVACIONES DE TALLAS':    p.obs_tallas ?? '',
    ...
    'OTRA ENFERMEDAD':            p.enfermedades_otra ?? '',
    ...
    'OTRO ANTECEDENTE FAMILIAR':  p.antecedentes_familiares_otra ?? '',
    ...
    'AUTORIZA TRATAMIENTO DE DATOS': bool(p.autoriza_datos),
    'DECLARA VERACIDAD':          bool(p.declara_veracidad),
    // La firma se guarda como imagen: en la hoja solo interesa si ya firmó.
    'FIRMÓ':                      p.firma_electronica ? 'SÍ' : 'NO',
    'FECHA DE CONSENTIMIENTO':    p.fecha_consentimiento ? new Date(p.fecha_consentimiento).toLocaleDateString('es-CO') : '',
  ...
  // El número de columnas cambia cuando se agregan campos, así que el rango del
  // filtro se calcula en vez de dejarlo fijo (antes estaba clavado en BW).
  const nCols = Object.keys(base[0] ?? {}).length || 1
  ws1['!autofilter'] = { ref: `A1:${XLSX.utils.encode_col(nCols - 1)}1` }
  ws1['!cols'] = [
    { wch: 28 },{ wch: 14 },{ wch: 16 },{ wch: 28 },{ wch: 26 },
    ...Array(Math.max(nCols - 5, 0)).fill({ wch: 18 }),
  ]
  ...
  const freqs: (string | number)[][] = [
    ...
    ...makeFreqBlock('TALLA CAMISA / CAMISETA', analytics.tallaCamisa),
