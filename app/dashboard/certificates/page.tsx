    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-white mb-1">Certificados</h1>
            <p className="text-slate-400 text-sm">{CERTS.length} certificados emitidos</p>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Total emitidos', value: CERTS.length, color: 'text-blue-400' },
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
              ...
              {filtered.map((c, i) => {
                ...
                return (
                  <motion.tr key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                    className="hover:bg-white/2 transition-colors cursor-pointer" onClick={() => setSelected(c)}>
                    ...
                    <td className="px-5 py-4 text-slate-300 text-sm max-w-[200px] truncate">{c.course}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5 text-slate-400 text-xs"><Calendar size={11} /> {c.issued}</div>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5 text-slate-400 text-xs"><Calendar size={11} /> {c.expires}</div>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <button className="text-slate-500 hover:text-blue-400 transition-colors" onClick={e => { e.stopPropagation(); setSelected(c) }}>
                        <Download size={15} />
                      </button>
                    </td>
                  </motion.tr>
        ...
        <div className="md:hidden divide-y divide-white/5">
          {filtered.map((c, i) => {
            ...
            const StIcon = st.icon
            return (
              <div key={c.id} className="p-4 hover:bg-white/2 transition-colors cursor-pointer" onClick={() => setSelected(c)}>
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="text-white font-semibold text-sm">{c.name}</div>
                    <div className="text-slate-500 text-xs mt-0.5 line-clamp-1">{c.course}</div>
                  </div>
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            className="bg-[#0D1629] border border-white/12 rounded-2xl w-full max-w-sm" onClick={e => e.stopPropagation()}>

            {/* Certificate card design */}
            <div className="bg-gradient-to-br from-blue-600/20 to-violet-600/20 border border-blue-500/20 rounded-t-2xl p-6 text-center">
              ...
              <div className="text-xs text-blue-300 font-semibold mb-1">CERTIFICADO DE COMPETENCIA</div>
              <h2 className="text-white font-black text-lg leading-snug">{selected.course}</h2>
            </div>

            <div className="p-6">
              <div className="space-y-3 mb-5">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400 flex items-center gap-1.5"><User size={12} /> Empleado</span>
                  <span className="text-white font-semibold">{selected.name}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400 flex items-center gap-1.5"><Calendar size={12} /> Emisión</span>
                  <span className="text-white">{selected.issued}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400 flex items-center gap-1.5"><Clock size={12} /> Vencimiento</span>
                  <span className="text-white">{selected.expires}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400 flex items-center gap-1.5"><Shield size={12} /> Código</span>
                  <span className="text-white font-mono text-xs">{selected.code}</span>
                </div>
              </div>
